...
class Decoder(nn.Module):
    ...
    def __init__(self, num_classes, in_planes, device):
        super(Decoder, self).__init__()
        self.num_classes = num_classes  # this is the output classes. So it includes the <EOS>.
        self.in_planes = in_planes
        self.sDim = 512
        self.attDim = 512
        self.max_len_labels = 200

        self.device = device
        self.decoder = DecoderUnit(sDim=self.sDim, xDim=in_planes, yDim=num_classes, attDim=self.attDim)
    ...
    def forward(self, x):
        ...
        for i in range(lengths):
            if i == 0:
                y_prev = torch.zeros(batch_size, device=self.device).fill_(
                    self.num_classes)  # the last one is used as the <BOS>.
            else:
                y_prev = targets[:, i - 1]
    ...
    def beam_search(self, x, beam_width):
        ...
        temp = torch.Tensor([i * beam_width for i in range(0, batch_size)]).long()
        temp = temp.to(self.device)
        sequence_scores.index_fill_(0, temp, 0.0)
        ...
        for i in range(self.max_len_labels):
            ...
            eos_indices = y_prev.view(-1, 1).eq(98)
        ...
        while t >= 0:
            ...
            eos_indices = stored_emitted_symbols[t].eq(98).nonzero()
    ...
    def predict(self, x):
        ...
        for i in range(self.max_len_labels):
            if i == 0:
                y_prev = torch.zeros(batch_size, device=self.device).fill_(self.num_classes)
            else:
                y_prev = predicted
